...
namespace {   // private module-only namespace
    ...
    socket_t hostname_connect(const std::string& hostname, int port) {
        ...
        for (const addrinfo* p = result; p != nullptr; p = p->ai_next) {
            ...
            if (connect(s, p->ai_addr, static_cast<int>(p->ai_addrlen)) != SOCKET_ERROR) {
                break;
            }
            closesocket(s);
            s = INVALID_SOCKET;
    ...
    class DummyWebSocket final : public easywsclient::WebSocket {
    public:
    ...
    public:
        ...
        {   // timeout in milliseconds
            ...
            if (timeout != 0) {
                ...
                select(static_cast<int>(sockfd + 1), &r_fds, &w_fds, nullptr, timeout > 0 ? &tv : nullptr);
            }
            while (true) {
                const int     N = static_cast<int>(rxbuf.size());
                rxbuf.resize(N + 1500);
                const ssize_t ret = recv(sockfd, reinterpret_cast<char*>(rxbuf.data()) + N, 1500, 0);
            ...
            while (!txbuf.empty()) {
                const int ret = ::send(sockfd, reinterpret_cast<char*>(txbuf.data()), static_cast<int>(txbuf.size()), 0);
                if (ret < 0 && (socketerrno == SOCKET_EWOULDBLOCK || socketerrno == SOCKET_EAGAIN_EINPROGRESS)) {
                    break;
                }
                if (ret <= 0) {
        ...
        void _dispatch(Callback_Imp& callable) override
        {
                ...
                // Adapt void(const std::string<uint8_t>&) to void(const std::string&)
            {
                ...
                explicit CallbackAdapter(Callback_Imp& callable) : callable(callable) {
                }

                void operator( )(const std::vector< uint8_t >& message) override
                {
	                const std::string stringMessage(message.begin(), message.end());
        ...
        virtual void _dispatchBinary(BytesCallback_Imp& callable) {
            ...
            while (true) {
                ...
                const uint8_t* data = rxbuf.data();   // peek, but don't consume
                ws.fin = (data[0] & 0x80) == 0x80;
                ws.opcode = static_cast<wsheader_type::opcode_type>(data[0] & 0x0f);
                ...
                if (ws.N0 < 126) {
                    ...
                    ws.N = 0;
                    ws.N |= static_cast<uint64_t>(data[2]) << 8;
                    ws.N |= static_cast<uint64_t>(data[3]) << 0;
                    i = 4;
                }
                else if (ws.N0 == 127) {
                    ws.N = 0;
                    ws.N |= static_cast<uint64_t>(data[2]) << 56;
                    ws.N |= static_cast<uint64_t>(data[3]) << 48;
                    ws.N |= static_cast<uint64_t>(data[4]) << 40;
                    ws.N |= static_cast<uint64_t>(data[5]) << 32;
                    ws.N |= static_cast<uint64_t>(data[6]) << 24;
                    ws.N |= static_cast<uint64_t>(data[7]) << 16;
                    ws.N |= static_cast<uint64_t>(data[8]) << 8;
                    ws.N |= static_cast<uint64_t>(data[9]) << 0;
                    i = 10;
                ...
                if (ws.mask) {
                    ws.masking_key[0] = static_cast<uint8_t>(data[i + 0]) << 0;
                    ws.masking_key[1] = static_cast<uint8_t>(data[i + 1]) << 0;
                    ws.masking_key[2] = static_cast<uint8_t>(data[i + 2]) << 0;
                    ws.masking_key[3] = static_cast<uint8_t>(data[i + 3]) << 0;
                }
                else {
                    ws.masking_key[0] = 0;
                    ws.masking_key[1] = 0;
                ...
                if (false) {
                    ...
                    if (ws.mask) {
                        for (size_t size = 0; size != ws.N; ++size) {
                            rxbuf[size + ws.header_size] ^= ws.masking_key[size & 0x3];
                        }
                    }
                    std::string basic_string(rxbuf.begin() + ws.header_size, rxbuf.begin() + ws.header_size + (size_t)ws.N);
                    sendData(wsheader_type::PONG, basic_string.size(), basic_string.begin(), basic_string.end());
                }
        ...
        void sendData(wsheader_type::opcode_type type, uint64_t message_size, Iterator message_begin, Iterator message_end) {
            ...
            // middleware:
            constexpr uint8_t masking_key[4] = { 0x12, 0x34, 0x56, 0x78 };
            ...
            if (useMask) {
                const size_t message_offset = txbuf.size() - message_size;
    ...
    easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask, const std::string& origin) {
        ...
        const socket_t s = hostname_connect(host, port);
        if (s == INVALID_SOCKET) {
        ...
        }
        {
            ...
            ::send(s, line, static_cast<int>(strlen(line)), 0);
            snprintf(line, 1024, "Sec-WebSocket-Extensions: deflate-stream\r\n");
            ::send(s, line, static_cast<int>(strlen(line)), 0);
        ...
        int flag = 1;
        setsockopt(s, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<char*>(&flag), sizeof(flag));   // Disable Nagle's algorithm
#ifdef _WIN32
        u_long on = 1;
        ...
        return new _RealWebSocket(s, useMask);
    }
...
namespace easywsclient {
    ...
    WebSocket::pointer WebSocket::create_dummy() {
        static auto dummy = static_cast<pointer>(new DummyWebSocket);
        return dummy;
    }
